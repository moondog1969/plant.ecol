...
\value{
A sorted table, and the order of sorted rows (with respect to the original order), is returned.
}
\references{
...
# Sorting a summarized releve from veg.table
data(dune.env)
order <- sortid(t(dune))$sort
veg.table(dune, dune.env$Management, row.order = order)
}
