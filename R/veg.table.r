veg.table <- function(Y, cat, min.const = .3, method = "aho", cover.only = FALSE, row.order = NULL)
{
  ...
  if(!is.null(row.order)){
    if(n != length(row.order)) warning(paste("row.order must have length", n))  
    cvr1 <- cvr1[row.order,]
    con1 <- con1[row.order,]
  }
...
if (method =="braun.n")
       ...
       cv2<-apply(cv3,2,function(x){replace(x,x>0.1&x<=1,1100)})
       cv2.1<-apply(cv2,2,function(x){replace(x,x>0&x<=0.1,1000)})
       cv1<-apply(cv2.1,2,function(x){replace(x,x==0,"0")})
       cv7<-apply(cv1,2,function(x){replace(x,x==1200,"7")})
       cv6<-apply(cv7,2,function(x){replace(x,x==1180,"6")})
       cv5<-apply(cv6,2,function(x){replace(x,x==1160,"5")})
       cv4<-apply(cv5,2,function(x){replace(x,x==1140,"4")})
       cv3<-apply(cv4,2,function(x){replace(x,x==1120,"3")})
       cv2<-apply(cv3,2,function(x){replace(x,x==1100,"2")})
       cv1<-apply(cv2,2,function(x){replace(x,x==1000,"1")})
       cover<-as.data.frame(cv1)
       }
