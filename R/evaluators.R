...
{
    if(inherits(cat, "factor")) 
        cat <- as.numeric(cat)
    numclu <- max(cat)
    call <- match.call()
    
    if(!inherits(dist, "dist")) stop("you must supply an object of class dist")
...
Cindex.eval <- function (cat, Y, index = "bray", stopif1 = FALSE) 
{
  ...
  for (i in 1:max(cat)) {
    tveg <- Y[cat==i,] 
    if(is.null(nrow(tveg == 1)) & stopif1 == FALSE)
    { tmp <- 0; vd[i] <- 0; size <- 1; cd[i] <- 0}
    if(is.null(nrow(tveg == 1)) & stopif1 == TRUE)
    { stop("Cluster with only one object")}
    if(!is.null(nrow(tveg > 1))){ tmp <- vegdist(tveg,index)
    vd[i] <- sum(tmp)
    size <- attr(tmp,'Size')
    cd[i] <- (size^2 - size)/2}
  }
